...
If you provide the inverter Web API technician password in the integration setup, the integration can:
...
The Web API username is fixed to `technician` — the local technician account on your inverter. The technician password is set during commissioning by your installer and is different from your Solar.web cloud login (e.g. https://www.solarweb.com/) and different from the `customer` account password.
...
If an entry has no valid stored Web API token for the configured host, Home Assistant raises a Repairs item that lets you review the host settings and enter the technician password to mint a new token.
...
### Inverter API Controls

| Entity               | Description                                                                                                                                           |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| Watt Peak Reference  | Sets the watt peak reference value used for the power limits visualization (`wattPeakReferenceValue` via `/api/config/limit_settings/powerLimits`).   |
| Solar API            | Enables or disables the Fronius Solar API v1 on the inverter. Disabling is recommended on firmware below 1.40.7-1 to avoid a known memory leak issue. |
| Reset Modbus Control | Sends a Modbus reset command to the inverter (`/api/commands/ModbusReset`). Diagnostic entity.                                                        |
...
| Entity                                       | Description                                                                                                                                                                                                        |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Grid status                                  | Grid status based on meter and inverter frequency. If inverter frequency is around 53 Hz it is running in off-grid mode, normally 50 Hz. When the inverter is sleeping the meter frequency is checked for connection. |
